...
A Sol MAX design worker may implement its design when separating design from code would damage the feedback loop. Once the design stabilizes, Main may split repetitive expansion, non-visual wiring, tests, and cleanup to Luna. Mixed tasks may run a Sol design lane beside disjoint Luna lanes when ownership is safe.
